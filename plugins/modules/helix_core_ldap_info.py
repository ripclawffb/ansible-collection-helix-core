...
DOCUMENTATION = '''
---
module: helix_core_ldap_info

short_description: Get LDAP configuration information from Perforce Helix Core

version_added: "1.1.0"

description:
    - "Retrieves LDAP configuration specifications from Perforce Helix Core."
    - "When C(name) is provided, returns a single LDAP config spec."
    - "When C(name) is omitted, returns a list of all LDAP configurations."
...
info:
    description:
        - When C(name) is provided, a dict with the LDAP spec fields.
        - When C(name) is omitted, a list of LDAP configuration dicts.
    returned: success
    type: raw
'''
...
def run_module():
    ...
    try:
        if module.params['name']:
            spec = p4.fetch_ldap(module.params['name'])
            result['info'] = dict(spec)
        else:
            result['info'] = p4.run_ldaps()
