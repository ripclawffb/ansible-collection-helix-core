...
DOCUMENTATION = '''
---
module: helix_core_remote_info

short_description: Get remote specification information from Perforce Helix Core

version_added: "1.3.0"

description:
    - "Retrieves remote specifications from Perforce Helix Core."
    - "When C(name) is provided, returns a single remote spec."
    - "When C(name) is omitted, returns a list of all remotes."
...
seealso:
    - name: Helix Core Remote
      description: "Manage remote specs"
...
info:
    description:
        - When C(name) is provided, a dict with the remote spec fields.
        - When C(name) is omitted, a list of remote summary dicts.
    returned: success
    type: raw
'''
...
def run_module():
    ...
    try:
        if module.params['name']:
            spec = p4.fetch_remote(module.params['name'])
            result['info'] = dict(spec)
        else:
            result['info'] = p4.run_remotes()
