...
description:
    ...
    - "When C(name) is provided, returns a single client spec."
    - "When C(name) is omitted, returns a list of all clients."
...
info:
    description:
        - When C(name) is provided, a dict with the client spec fields.
        - When C(name) is omitted, a list of client summary dicts.
    returned: success
    type: raw
'''
...
def run_module():
    ...
    try:
        if module.params['name']:
            spec = p4.fetch_client(module.params['name'])
            result['info'] = dict(spec)
        else:
            result['info'] = p4.run_clients()
