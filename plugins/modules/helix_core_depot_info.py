...
description:
    ...
    - "When C(name) is provided, returns a single depot spec."
    - "When C(name) is omitted, returns a list of all depots."
...
info:
    description:
        - When C(name) is provided, a dict with the depot spec fields.
        - When C(name) is omitted, a list of depot summary dicts.
    returned: success
    type: raw
'''
...
def run_module():
    ...
    try:
        if module.params['name']:
            spec = p4.fetch_depot(module.params['name'])
            result['info'] = dict(spec)
        else:
            result['info'] = p4.run_depots()
